...
// Credit-tenant NNN c-store evaluation framework.
// Auto-injected into the AI prompt whenever the deal is a convenience store.
const CSTORE_EVALUATION_FRAMEWORK = `CREDIT-TENANT NNN C-STORE EVALUATION FRAMEWORK:
This is a credit-tenant NNN convenience store deal. Apply the following mindset to the evaluation:

CAP RATE CONTEXT: Cap rates of 5.25–6.25% are correct for corporate-guaranteed absolute NNN c-stores (Royal Farms, Sheetz, Wawa, etc.). The lower yield reflects the tenant IS the corporation — no franchisee layer, no guaranty burnoff, revenue backed by hundreds of millions to billions. Do not flag a 5.5% cap as "below target" the same way you would for a franchisee deal. The risk profile is fundamentally different.

NEGATIVE LEVERAGE: If the investor's buy box was set for franchisee deals (65% LTV / 7% interest), it will show negative leverage on a 5–6% cap. That is NOT a disqualifier — it signals a different financing structure. Credit-tenant c-store lenders typically offer 1.15x DSCR, rates 75–100 bps below market, and loan terms matching the primary lease. Evaluate total return (cash flow + principal paydown + depreciation + rent escalation + appreciation), not cash-on-cash alone. Unlevered IRR on a well-structured credit c-store at 5.5% entry typically lands 7–8%.

WALT FIRST: Lease term remaining is the primary risk variable. With 10+ years of primary term and corporate guaranty, the income stream is near-institutional quality. Under 5 years primary term, the tenant controls the exit — if they walk, the investor owns a gas station with fuel infrastructure and limited re-tenanting options. Evaluate accordingly.

RENT ESCALATIONS: 10% bumps every 5 years compound to ~46% NOI growth over 20 years. 3% at each option barely keeps pace with inflation. This structural difference is a material factor in long-term return — not a minor footnote.

LEASEHOLD VS. FEE SIMPLE: If the deal is a ground lease, sublease, or leasehold position, the investor owns an income stream — not the land. This creates: fewer buyers on exit, lender haircuts on LTV, value erosion as term burns, and a potential second capital event to convert to fee simple. Flag this clearly if present. Fee simple (owning land + building outright) is structurally superior.

PORTFOLIO ROLE: This asset type is a wealth-preservation anchor, not a yield engine. Its value is certainty of income over 15–20 years. A franchisee TLE deal at 7.5–8% and a corporate c-store at 5.75% are not competing — they serve different functions in a portfolio.`;

function isCStore(deal: { assetType?: string | null }): boolean {
...
export async function runGapAnalysis(
  ...
  const cstoreSection = isCStore(deal) ? `\n${CSTORE_EVALUATION_FRAMEWORK}\n` : "";
...
${contextSection}${cstoreSection}${flexInstruction}${scoreSuppressionInstruction}${categoryInstruction}
Analyze this deal:
